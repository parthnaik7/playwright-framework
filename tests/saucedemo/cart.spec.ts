...
test.describe('SauceDemo — Add to Cart', () => {
  ...
  test('should add a single product to cart and update badge @smoke @regression', async () => {
  ...
  test('should add multiple products and show correct badge count @regression', async () => {
    ...
    log.info(
      `✅ Cart badge shows ${productsToAdd.length} after adding ${productsToAdd.length} products`,
    );
  });

  test('should allow adding all 6 products to cart @regression', async () => {
  ...
  test('should add two products then remove one and badge reflects correct count @regression', async () => {
...
test.describe('SauceDemo — Cart Page', () => {
  ...
  test('should navigate to cart page and display added items @smoke @regression', async () => {
  ...
  test('should show an empty cart when no items added @regression', async () => {
  ...
  test('should remove an item from the cart page @regression', async () => {
    await inventoryPage.addProductsToCart([PRODUCTS.BACKPACK, PRODUCTS.BIKE_LIGHT]);
    await inventoryPage.header.goToCart();
  ...
  test('should persist cart items when navigating back to inventory @regression', async () => {
    await inventoryPage.addProductToCart(PRODUCTS.BACKPACK);
    await inventoryPage.assertCartBadgeCount(1);
  ...
  test('should display correct item names and prices in cart @regression', async () => {
    await inventoryPage.addProductsToCart([PRODUCTS.BACKPACK, PRODUCTS.BIKE_LIGHT]);
    await inventoryPage.header.goToCart();
    await cartPage.assertOnCartPage();
...
test.describe('SauceDemo — Inventory Sort', () => {
  ...
  test('should sort products A to Z by default @regression', async () => {
